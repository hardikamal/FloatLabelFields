...
@IBDesignable class FloatLabelTextView: UITextView {
    ...
    fileprivate var isSetOnce = false
    ...
    @IBInspectable var borderColour:UIColor = UIColor.gray {
        didSet {
            if !isFirstResponder {
    ...
    @IBInspectable var borderActiveColour:UIColor! {
        didSet {
            if isFirstResponder {
                borderColor = titleActiveTextColour
            }
        }
    }
    ...
    override func layoutSubviews() {
        ...
        if isResp && !text.isEmpty {
            title.textColor = titleActiveTextColour
            borderColor = borderActiveColour
        } else {
            title.textColor = titleTextColour
            borderColor = borderColour
        }
        if !isSetOnce {
            setBottomBorder(color: borderColor)
        }else{
    ...
    fileprivate func setup() {
        ...
        //setBottomBorder(color: UIColor.gray)
        // Observers
        if !isIB {
            let nc = NotificationCenter.default
    ...
    func setBottomBorder(color:UIColor)
    {
        ...
        borderView.frame = CGRect(x: self.frame.origin.x, y: self.frame.origin.y+self.frame.height - width,   width:  self.frame.size.width, height: width)
